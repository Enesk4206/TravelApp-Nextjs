    ...
    <div className={` ${navBg ? "bg-blue-950 shadow-md" : "fixed"}   transition-all duration-200 h-[12vh] z-[1000] fixed w-full`}>
      <div className='flex items-center h-full justify-between w-[90%] xl:w-[80%] mx-auto'>
        ...
        <div className='hidden lg:flex items-center space-x-10'>
          {navLinks.map((link) => {
            return <Link href={link.url} key={link.id}>
              <p className='relative text-white text-base font-bold w-fit  block after:block after:content-between
              after:absolute after:h-[3px] after:bg-yellow-300 after:w-full after:scale-x-0 after:hover:scale-x-100
              after:transition-all duration-300 after:origin-right'
              >
                {link.label}
              </p>
            </Link>
        ...
        <div className='flex items-center space-x-4'>
          ...
          {/* burgers menu */}
          <HiBars3BottomRight onClick={openNav}  className='size-8 cursor-pointer text-white lg:hidden'/>
        </div>
      </div>
    </div>
  )
